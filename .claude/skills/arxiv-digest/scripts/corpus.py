...
# Library papers want abstracts + keywords (for the relevance profile) on top of the usual metadata.
LIB_FIELDS = (A.DEFAULT_FIELDS + ",abstract,keyword")
...
def write_summary(path, lib_name, papers, siblings, cfg):
    ...
    for r in papers_sorted:
        ...
        lines.append(f"- **{yr}** · {C.short_authors(r)} · {title}  _( {nsib} siblings )_")
...
def main():
    ...
    sys.stderr.write(f"[corpus] finding siblings for {len(todo)} papers (window +/-{sib_window} yr, {sib_rows} rows each) ...\n")
    siblings = {}
    for i, paper in enumerate(todo, 1):
        sibs = fetch_siblings(token, paper, cache, sib_rows, sib_window, user_bibcodes,
